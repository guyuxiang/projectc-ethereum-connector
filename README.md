...
projectc-ethereum-connector is a HTTP apiserver framework based on [Gin](https://github.com/gin-gonic/gin).

## Introduction

`projectc-ethereum-connector`是一个基于[gin](https://github.com/gin-gonic/gin)框架写的ApiServer框架，主要用于企业生产环境中的快速开发
...
* 3、支持dump-goroutine-stack-traces
  ...
  github.com/guyuxiang/projectc-ethereum-connector/pkg/util.dumpStacks()
          /root/go/src/github.com/guyuxiang/projectc-ethereum-connector/pkg/util/trap.go:23 +0x6d
  github.com/guyuxiang/projectc-ethereum-connector/pkg/util.SetupSigusr1Trap.func1(0xc000332240)
          /root/go/src/github.com/guyuxiang/projectc-ethereum-connector/pkg/util/trap.go:16 +0x34
  created by github.com/guyuxiang/projectc-ethereum-connector/pkg/util.SetupSigusr1Trap
          /root/go/src/github.com/guyuxiang/projectc-ethereum-connector/pkg/util/trap.go:14 +0xab
  ...
  github.com/gin-gonic/gin.(*Engine).Run(0xc000394000, 0xc000355f48, 0x1, 0x1, 0x0, 0x0)
          /root/go/src/github.com/guyuxiang/projectc-ethereum-connector/vendor/github.com/gin-gonic/gin/gin.go:294 +0x140
  main.main()
          /root/go/src/github.com/guyuxiang/projectc-ethereum-connector/cmd/main.go:22 +0x2c4
...
`projectc-ethereum-connector`框架的核心就是pkg包，下面主要针对该包结构进行描述：
...
* step1 - 替换项目名称
  ...
  实际使用中，通常需要将`projectc-ethereum-connector`替换成业务需要的后台server名称，可以执行如下命令：

  ```bash
  $ grep -rl projectc-ethereum-connector . | xargs sed -i 's/projectc-ethereum-connector/youapiserver/g' 
  ```
...
* step3 - 启动服务  
  ...
  $ docker tag guyuxiang/projectc-ethereum-connector:v0.1.0 xxx/guyuxiang/projectc-ethereum-connector:v0.1.0
  $ docker push xxx/guyuxiang/projectc-ethereum-connector:v0.1.0
  # Update the deployment to use the built image name
  $ sed -i 's|REPLACE_IMAGE|xxx/guyuxiang/projectc-ethereum-connector:v0.1.0|g' hack/deploy/deployment.yaml
  # create service 
